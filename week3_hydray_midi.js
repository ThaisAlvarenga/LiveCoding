...
// based on the brightness, luma will make things be transparent or not

solid().out()
hush()

// example of using audio input in hydra (need to launch from terminal on mac)
osc(200,0.01,1).rotate(1).layer(osc(30,0,1).luma(0.2,.2).color(0,0,0,1)).layer(osc(30,0,1).luma(0.5,0.01)).modulate(noise(()=>(cc[0]+cc[1])*3,.1)).out()
